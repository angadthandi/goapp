...
import (
	...
	"github.com/angadthandi/goapp/api/ws"
	...
	"github.com/angadthandi/goapp/util/genuuid"
...
func (c *Client) ReadPump(
	...
	for {
		...
		go ws.API(
			dbRef, reg, configData, c.clientID, message,
		)
	}
}
...
func ServeWs(
	...
	userID, err := auth.ParseToken(configData.JWTAuthSecret, tokenStr)
	if err != nil {
		log.Errorf("unable to parse token: %v", err)
		return
	}

	// TODO validate token before establishing ws conn
	// might be done before this gets called
	...
	// new goroutines.
	go client.WritePump()
	go client.ReadPump(dbRef, reg, configData)
}
